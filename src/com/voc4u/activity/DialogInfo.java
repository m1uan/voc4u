...
public class DialogInfo {
	...
	public static void setup(final Context context, final String type,
			final Dialog dialog) {
		Resources r = context.getResources();
		int titleid = r.getIdentifier("form_" + type, "string",
				context.getPackageName());
		int textid = r.getIdentifier("information_" + type, "string",
				context.getPackageName());
		...
		final CheckBox chkNSM = (CheckBox) dialog
				.findViewById(R.id.chkNoShowMore);
		...
		if (type.contentEquals(TYPE_TRAIN)) {
			Button btn = (Button) dialog.findViewById(R.id.btnDictionary);
			btn.setVisibility(View.VISIBLE);
			btn.setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View v) {
					WordController.getInstance(context).showWordsMenu();
				}
			});
		}
	}
	...
	public static boolean GetChecked(final String type) {
		...
		} else if (type.contentEquals(TYPE_DASHBOARD)) {
			return CommonSetting.NSMDashboard;
		} else
	...
	private static void SetupCommonSetting(final String type, boolean isChecked) {
		if (type.contentEquals(TYPE_INIT)) {
			...
			CommonSetting.NSMDashboard = isChecked;
		}
	}
}
